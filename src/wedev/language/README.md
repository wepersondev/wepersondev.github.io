...
머 이유가 이거 하나만이겠냐만은, 언제부터인가 국내 소프트웨어 시장이 Java로 획일화 되었고 이후 넘처나는 Java 개발자들이 그 개발 방식으로 그대로 이어가고 있는 것이 현실입니다. Java는 획일화되고 정형적인 패턴을 짜기에 좋은 보수적인 기술이지 발전하는 기술이 아니다. 이미 Java의 트렌드가 죽어가고 있으며 **Java가 그렇게 좋은 언어라면 안드로이드가 왜 Java를 버리고 [Kotlin](https://kotlinlang.org/)으로 넘어갔을가?** 다른 것도 할줄 알고 Java도 잘하면 좋은거지만 Java만을 메인 스택으로 하는 기술자들은 좀 생각해봐야 할 듯 합니다.
...
결론적으로 저희는 풀스택, 멀티 플렛폼, 효율성, 안정성, 협업, 스타트업, 러닝커브, 인력 수급등 여러가지를 고려하여 TypeScript을 주로 사용하고 일부 JavaScript와 Python을 사용합니다. 각 플렛폼별 예외처리를 위해서 Java/Kotlin나 Swift등도 같이 사용합니다. 이러한 언어를 사용하게 된 배경과 프레임워크등을 이어서 설명 예정입니다.
