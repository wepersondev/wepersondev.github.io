...
:::tip
<small>
머 비교만 하면 다 비슷하다고 하는데 도데체 필자는 클라우드 서비스를 써보기나 한것인가?
10년이 넘게 3사 클라우드를 통해 모두 상용 서비스를 구축하였고 클라우드 초기에는 지금 보이는 서비스들의 1/3도 되지 않았습니다. 없으면 없는대로 생기면 생기는 대로 사용해서 적용하고 문제가 있으면 우회 방법 찾아서 해결하고를 반복하다보니 이제 거시적으로는 대부분 비슷하게만 보입니다. 디테일에 대해서는 경험적으로 명확하게 알고 있으며 프로젝트 따라서 방법을 달리하여 적용합니다. 포스팅을 계속 읽어보시면 디테일에 대해서 언급되어 있습니다.
</small>
:::
